...
import { X, ChevronLeft, ChevronRight, Share, Package, ExternalLink, MessageCircle, Circle } from "lucide-react";
...
export function ShowcaseModal({
    ...
    // 1. STATE FOR PRODUCT NAVIGATION
    const [currentIndex, setCurrentIndex] = useState(initialStartIndex);
    
    // 2. STATE FOR IMAGE NAVIGATION (New)
    ...
    // Get the specific image to display based on imageIndex
    ...
    const nextImage = (e: React.MouseEvent) => {
        ...
        if (hasMultipleImages && imageIndex < currentItem.images.length - 1) {
            ...
            setImageIndex(0); // Loop back to start
        }
    };
    ...
    const prevImage = (e: React.MouseEvent) => {
        ...
        if (hasMultipleImages && imageIndex > 0) {
            ...
            setImageIndex(currentItem.images.length - 1); // Loop to end
        }
    };
                        ...
                        <div className="relative w-full md:w-[45%] h-64 md:h-full bg-gray-50 flex items-center justify-center overflow-hidden border-r border-gray-100 shrink-0 select-none">
                            ...
                            {displayImage ? (
                                <div className="relative w-full h-full p-4 flex items-center justify-center">
                                    ...
                                    {/* --- INTERNAL IMAGE NAVIGATION (If > 1 Image) --- */}
                                    {hasMultipleImages && (
                                        <>
                            ...
                            {/* --- OUTER PRODUCT NAVIGATION (Switch between Products) --- */}
                            {/* We position these at the absolute edges to distinguish from image nav */}
                            {items.length > 1 && (
                                <>
                                    <Button
                                        variant="ghost"
