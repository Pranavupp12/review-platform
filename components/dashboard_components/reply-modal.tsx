      ...
      <DialogContent className="sm:max-w-[500px] p-6">
        <DialogHeader className="mb-4">
          <DialogTitle className="flex items-center gap-2 text-xl font-semibold text-gray-900">
            ...
            <TranslatableText text="Response from" /> {companyName}
          </DialogTitle>
          <DialogDescription className="text-sm text-gray-500 mt-1.5 ml-1">
            {replyDate 
              ? <><TranslatableText text="Replied on" /> {format(new Date(replyDate), "MMMM d, yyyy")}</>
              : <TranslatableText text="Date not available" />}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-100 relative">
            ...
            <div className="text-gray-700 leading-relaxed italic relative z-10 px-2 pt-1">
              {/* ✅ Translatable Reply Text */}
              <TranslatableText text={replyText} />
            </div>
        </div>
