...
interface FlaggedReviewCardProps {
  review: {
    ...
    adminMessage: string | null; // <--- The Admin Note
    company: {
      name: string;
      slug: string;
...
export function FlaggedReviewCard({ review }: FlaggedReviewCardProps) {
  return (
    <div className="bg-white rounded-lg p-6 border-2 border-orange-200 shadow-sm flex flex-col relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 bg-orange-50 border-b border-orange-100 p-3 flex items-start gap-3">
        ...
        <div>
           ...
           <p className="text-xs text-orange-700 mt-1">
             {review.adminMessage || "This review has been flagged. Please edit it to comply with our guidelines."}
           </p>
        </div>
      </div>
      ...
      {/* Standard Card Content (Simplified from MyReviewCard) */}
      ...
      <div className="mb-4 opacity-75">
        ...
        <h3 className="font-bold text-gray-900 mb-1 line-clamp-1">{review.reviewTitle}</h3>
        <p className="text-gray-600 text-sm line-clamp-2 italic">"{review.comment}"</p>
      </div>
