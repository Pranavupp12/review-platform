...
import { createBusinessUpdate } from "@/lib/updates-actions";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { Loader2, Upload, Plus, X, Lock } from "lucide-react";
...
interface AddBusinessUpdateModalProps {
  companyId: string;
  isLimitReached: boolean; // ✅ NEW PROP to control button state
}

export function AddBusinessUpdateModal({ companyId, isLimitReached }: AddBusinessUpdateModalProps) {
  const [open, setOpen] = useState(false);
  
  // Use React Action State
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {isLimitReached ? (
           // 🔒 LOCKED BUTTON: Disabled and greyed out
           <Button disabled className="bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed flex items-center">
              <Lock className="mr-2 h-4 w-4" /> Monthly Limit Reached
           </Button>
        ) : (
           // ✅ ACTIVE BUTTON: Standard styling
      ...
      {/* Only render content if limit is NOT reached (extra safety, though button is disabled) */}
      {!isLimitReached && (
